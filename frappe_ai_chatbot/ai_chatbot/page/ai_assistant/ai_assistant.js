...
class AIAssistant {
	...
	create_message_element(msg) {
		...
		if (msg.tool_calls && msg.tool_calls.length > 0) {
			const tool_container = $('<div></div>');
			msg.tool_calls.forEach(tool => {
				tool_container.append(this.create_tool_call_element(tool));
			});
			message_div.find('.ai-message-content').append(tool_container);
		}
	...
	async send_message() {
		...
		eventSource.addEventListener('tool_result', (e) => {
			try {
				const data = JSON.parse(e.data);
				const tool_index = data.tool_index || tool_calls.length - 1;
				
				if (tool_calls[tool_index]) {
					// Update tool with result
					tool_calls[tool_index].result = data.result;
