		...
		try:
			...
			for event in self.adapter.stream_chat(
				...
				if event.get("type") == "tool_call" and self.settings.enable_tool_calling:
					...
					# Note: In streaming mode, tool results don't automatically
					# continue the conversation. Frontend can choose to:
					# 1. Display results to user
					# 2. Send results back to LLM for further processing
				
				else:
					# Forward all other events (content, error, done, etc.)
					yield event
